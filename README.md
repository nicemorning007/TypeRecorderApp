...
- 每小时按键趋势：柱状图展示今日 0 点到 23 点的敲击次数，折线图展示昨日同小时统计。
- 最近输入：按全局事件 ID 倒序展示最近输入。
- 紧凑键盘热力图：快速查看高频按键。

### 分时统计

分时统计页会把今天和昨天的按键事件分别按自然小时聚合成 24 个桶：

- 柱形图展示今日每小时按键次数。
- 折线图展示昨日每小时按键次数，方便和今日同小时对照。
- 页面会展示今日分时总按键、今日最忙小时和今日升温最快的小时。
- App 会根据今日敲击时段分布给用户生成趣味头衔，例如深夜输入较多时显示“夜猫子敲击官”。
- 分时统计固定使用自然日今天 / 昨天，不受“每日 0 时自动重置统计”开关影响。
...
- 柱状图只统计今天 0 点到 23 点的按键。
- 折线图只统计昨天 0 点到 23 点的按键。
- “每日 0 时自动重置统计”只影响总览、热力图等统计口径，不影响分时统计。
- “排除功能键”和“统计中保留 Esc / Return / Delete”会同步影响小时图表。
- 0 点没有同一天内的上一小时，因此趋势增量从 0 开始计算，避免跨天回绕造成误读。
